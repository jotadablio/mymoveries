...
const MOVIES_STORAGE_KEY = '@MyMoveries:movies';
...
const MoviesListScreen = ({ navigation }) => {
    ...
    const loadMovies = async () => {
      try {
        ...
        if (storedMovies !== null) {
          ...
          setMovies([
            { id: '1', name: 'Django Livre', favorite: false, watched: false },
            { id: '2', name: 'Interestelar', favorite: false, watched: false },
            { id: '3', name: 'Clube da ****', favorite: false, watched: false },
          ]);
        }
      } catch (error) {
        console.error('Failed to load movies from storage', error);
        Alert.alert('Erro', 'Não foi possível carregar os filmes salvos.');
        setMovies([
            { id: '1', name: 'Django Livre', favorite: false, watched: false },
            { id: '2', name: 'Interestelar', favorite: false, watched: false },
            { id: '3', name: 'Clube da ****', favorite: false, watched: false },
  ...
  const shareAsText = async () => {
    ...
    try {
      let textContent = `Lista de Filmes\n\n`;
  ...
  const shareAsPDF = async () => {
    ...
    try {
      let htmlContent = `
        <!DOCTYPE html><html><head><meta charset="utf-8"><title>Lista de Filmes</title><style>body{font-family:Arial,sans-serif;margin:20px;color:#333}h1{color:#4A0000;text-align:center;margin-bottom:30px}.item{padding:10px 0;border-bottom:1px solid #ddd}.favorite{color:#FFD700}.watched{color:#4CAF50}.footer{margin-top:30px;text-align:center;font-size:12px;color:#777}</style></head><body><h1>Lista de Filmes</h1>`;
      movies.forEach((item, index) => {
      ...
      <LinearGradient
        ...
        <SafeAreaView style={styles.safeArea}>
          ...
          <FlatList
            ...
            ListEmptyComponent={<Text style={styles.emptyListText}>Nenhum filme na lista ainda.</Text>}
          />
          ...
          {showAddInput && (
            ...
            >
              <TextInput
                ...
                blurOnSubmit={false}
              />
              <TouchableOpacity style={styles.addConfirmButton} onPress={addMovie}>
          ...
          <EditItemsModal
            ...
            onRename={(id, newName) => {
                const success = renameMovie(id, newName);
            ...
            <View style={styles.modalContainer}>
              <View style={styles.modalContent}>
                ...
                <Text style={styles.aboutText}>
                  MyMoveries é um aplicativo para organizar suas listas de filmes e séries favoritas.
                </Text>
                <Text style={styles.aboutText}>
                  Versão 2.2.0
                </Text>
